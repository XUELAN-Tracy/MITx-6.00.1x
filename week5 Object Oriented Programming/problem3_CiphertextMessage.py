...
class CiphertextMessage(Message):
    ...
    def decrypt_message(self):
        ...
        '''
        best_counter = 0    
        wordList = Message.get_valid_words(self)
        
        for s in range(26):
            valid_counter = 0
            message_list = Message.apply_shift(self, s).split(' ')
            for word in message_list:
                if is_word(wordList, word) is True:
                    valid_counter += 1
                if valid_counter > best_counter:
                    best_counter = valid_counter
                    shift_value = s
                    message_decrpted = Message.apply_shift(self, shift_value)
        return (shift_value, message_decrpted)
